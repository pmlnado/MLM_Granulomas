...
```{r}
model_nested_offset <- glmer(rep_tb ~ CD4_prop + (1 | mouse/lesion) + offset(log(total_tb)), data = granuloma_data, family = poisson())
...
```{r}
model_mouse <- glmer(rep_tb ~ CD4_prop + (1 | mouse), data = granuloma_data, family = poisson())
...
model_mouse_offset <- glmer(rep_tb ~ CD4_prop + (1 | mouse) + offset(log(total_tb)), 
                           data = granuloma_data, 
                           family = poisson())
...
model_nb <- glmer.nb(rep_tb ~ CD4_prop + (1 | mouse) + offset(log(total_tb)), data = granuloma_data)
...
# Lesion nested within mouse
```{r}
model_nested <- glmer(rep_tb ~ DAPI_prop + (1 | mouse/lesion), data = granuloma_data, family = poisson())
...
# Lesion nested within mouse + offset to account for total Mtb
```{r}
model_nested_offset <- glmer(rep_tb ~ DAPI_prop + (1 | mouse/lesion) + offset(log(total_tb)), data = granuloma_data, family = poisson())
...
# Model accounting for mouse variance
```{r}
model_mouse <- glmer(rep_tb ~ DAPI_prop + (1 | mouse), data = granuloma_data, family = poisson())
...
```{r}
# Fitting the model with the offset
model_mouse_offset <- glmer(rep_tb ~ DAPI_prop + (1 | mouse) + offset(log(total_tb)), 
                           data = granuloma_data, 
                           family = poisson())
...
```{r}
# Fit the negative binomial model
model_nb <- glmer.nb(rep_tb ~ DAPI_prop + (1 | mouse) + offset(log(total_tb)), data = granuloma_data)
