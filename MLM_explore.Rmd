...
```{r}
granuloma_data <- read.csv("/Users/PabloM/Documents/HenaoLabDocuments/R_Projects/Granuloma_MLM/IHC_ISH_clean_data_WIP 1.csv")
...
model <- lmer(log_rep_tb ~ CD4_prop + (1 | lesion) + (1 | mouse), data = granuloma_data)

model1 <- lmer(log_rep_tb ~ CD4_prop + (1 | lesion/mouse), data = granuloma_data)

model.d <- lmer(log_rep_tb ~ CD4_prop + mouse + lesion + 
  mouse:lesion + CD4_prop:lesion + (lesion | mouse), data = granuloma_data)

# Summarize the model
summary(model)

residuals(model)
plot(fitted(model), residuals, main = "Residuals vs. Fitted", xlab = "Fitted values", ylab = "Residuals")
abline(h = 0, col = "red")  
